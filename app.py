...
T_min_C = st.sidebar.number_input("Min Temp (°C)", value=500, step=50)
T_max_C = st.sidebar.number_input("Max Temp (°C)", value=1000, step=50)
W_max_ideal = st.sidebar.number_input("Max ODE Integration Weight", value=5000, step=1000, help="This is the ideal weight limit. The final plotted weight will be this divided by 0.01.")
...
def dX_dW_ideal(W, X_array, T, FA0, ebs, P_CH40):
    X = X_array[0]
    if X >= 1.0: return [0.0]
    ...
    # Ideal ODE (No eta multiplier here)
    return [rA_intrinsic / FA0]

# Terminal event to stop integration if conversion reaches 99.9%
...
# ============================================================
@st.cache_data
def calculate_surface(T_min_C, T_max_C, T_op_C, Target_X, W_max_ideal, FA0, ebs, P_CH40):
    T_op_K = T_op_C + 273.15
    
    # 1. Calculate specific operating point
    sol_op = solve_ivp(
        fun=dX_dW_ideal, t_span=(0, W_max_ideal), y0=[1e-6], 
        args=(T_op_K, FA0, ebs, P_CH40), method='BDF',
    ...
    if np.max(X_op_profile) >= Target_X:
        # Interpolate the ideal weight, then divide by 0.01
        ideal_target_w = np.interp(Target_X, X_op_profile, W_op_profile)
        W_op_target = float(ideal_target_w) / 0.01

    # 2. Generate 3D Surface
    T_range_C = np.linspace(T_min_C, T_max_C, 30)  
    W_eval = np.linspace(0, W_max_ideal, 60) 
    T_mesh, W_ideal_mesh = np.meshgrid(T_range_C, W_eval)
    X_mesh = np.zeros_like(T_mesh)

    for i, T_C in enumerate(T_range_C):
        T_K = T_C + 273.15
        sol = solve_ivp(
            fun=dX_dW_ideal, t_span=(0, W_max_ideal), y0=[1e-6], t_eval=W_eval,      
            args=(T_K, FA0, ebs, P_CH40), method='BDF',
